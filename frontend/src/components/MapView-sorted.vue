...
function collectNearbyPoints(lon, lat, options = {}) {
  ...
  for (const ds of datasets.value) {
    ...
    for (const f of feats) {
      ...
      if (d <= maxDistance) {
        // 名稱與內容格式：景點用 地點名稱+地址，施工用 AP_NAME + PURP
...
function ensureNearbyCircleLayer() {
  ...
  if (!map.getLayer('nearby-points-highlight-symbol')) {
    ...
    try {
  if (!map.hasImage || !map.hasImage('construction-icon-yellow')) {
        const img = new Image()
        img.crossOrigin = 'anonymous'
        img.onload = () => {
          try { map.addImage('construction-icon-yellow', img) } catch (_) {}
          addRiskLayer()
        }
  img.src = yellowRiskIconUrl
      } else {
        addRiskLayer()
      }
    } catch (_) {
...
function searchInAttractionDataset(kw) {
  ...
  const f = feats.find(f => {
    const name = String(f?.properties?.['地點名稱'] || '')
